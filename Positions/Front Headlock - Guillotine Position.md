---
category: "Scramble / Transition"
dominance: 0
position_id: front_headlock
grapplemap_variations: 3
tags: [position, scramble]
---

# Front Headlock / Guillotine Position

**Category:** Scramble / Transition

## How to Identify

One person has their arm wrapped around the OTHER person's HEAD/NECK from the front. The head is trapped under the armpit. Often the defender's head is down. Can be standing or on the ground. The controller's body is sprawled back.

## Techniques from here
- [[Guillotine choke]]
- [[D'Arce choke]]
- [[Anaconda choke]]
- [[Snap to front headlock]]
- [[Go-behind]]

## Additional Techniques (GrappleMap)
- properties: bottom

## Transitions
**To:** [[Side Control (Top)]], [[Back Mount / Back Control]], [[Closed Guard (Bottom)]], [[Submission Defence (escaping)]]
**From:** [[Clinch / Collar Tie]], [[Turtle Attack (Top)]], [[Dogfight / Underhook Battle]], [[Wrestling Scramble]]

## Known Variations (GrappleMap)

*3 variations documented in GrappleMap database*

- bottom coming up into guillotine
- arm-in guillotine
- arm-out guillotine

## Instructional References

- Ryan Hall "Arm Triangles", Volume 3: Troubleshooting, Transitions and Drills, Technique 10: The Brabo Roll Under @ 46:35

## My Notes

<!-- Your observations, what works for you, what to drill -->
